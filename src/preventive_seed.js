...
} = require("./models/GeneralTaskList");
...
const funcLocData = require(
  path.join(__dirname, "..", "data", "functional_locations.json"),
);
const sapEquipmentData = require(
  path.join(__dirname, "..", "data", "sap_equipment.json"),
);
const taskListData = require(
  path.join(__dirname, "..", "data", "general_task_lists.json"),
);
const mappingData = require(
  path.join(__dirname, "..", "data", "equipment_interval_mappings.json"),
);
const sipilData = require(
  path.join(__dirname, "..", "data", "sipil_funcloc_mappings.json"),
);
...
const plants = [
  ...
  {
    plantId: "I-22L004",
    plantName: "Cipasauran",
    shortName: "Cipasauran",
    city: "Serang",
    centerLat: null,
    centerLon: null,
    zoom: 17,
  ...
  {
    plantId: "P-22L006",
    plantName: "WTP Cidanau",
    shortName: "WTP Cidanau",
    city: "Cilegon",
    centerLat: null,
    centerLon: null,
    zoom: 17,
  ...
  {
    plantId: "P-22L007",
    plantName: "WTP Krenceng",
    shortName: "WTP Krenceng",
    city: "Cilegon",
    centerLat: null,
    centerLon: null,
    zoom: 17,
...
async function main() {
  ...
  const existingPlants = await Plant.findAll({
    attributes: ["plantId", "centerLat", "centerLon"],
  });
  for (const p of existingPlants) {
    ...
    if (lat != null || lon != null)
      plantCoordSnapshot.set(p.plantId, { centerLat: lat, centerLon: lon });
  }
  if (plantCoordSnapshot.size > 0) {
    console.log(
      `  ✓  Snapshotted coords for ${plantCoordSnapshot.size} plant(s)`,
    );
  }
    ...
    for (const field of PROTECTED_FIELDS) {
      ...
      values[field] =
        row[field] !== null && row[field] !== undefined
          ? parseFloat(row[field]) || row[field]
          : null;
    }
  ...
  if (protectedSnapshot.size > 0) {
    console.log(
      `  ✓  Snapshotted protected fields for ${protectedSnapshot.size} equipment row(s)`,
    );
  }
  ...
  const plantNameMap = Object.fromEntries(
    plants.map((p) => [p.plantId, p.plantName]),
  );
  added = 0;
  for (const se of sapEquipmentData) {
    ...
    if (se.category) {
      ...
      else if (["listrik", "electrical"].includes(cat))
        safeCategory = "Listrik";
      else if (["sipil", "civil"].includes(cat)) safeCategory = "Sipil";
      else if (["otomasi", "automation"].includes(cat))
        safeCategory = "Otomasi";
    }
    const resolvedPlantId =
      plantNameMap[se.plantId] !== undefined ? se.plantId : null;
    const defaults = {
    ...
    try {
      let instance = await Equipment.findOne({
  ...
  for (const m of mappingData) {
    try {
      await EquipmentIntervalMapping.create({
  ...
  if (protectedSnapshot.size > 0) {
    ...
    const missedNote =
      missed > 0 ? ` — ${missed} ID(s) no longer exist, skipped` : "";
    console.log(
      `  ✓  protected     (${restored} equipment row(s) fields restored${missedNote})`,
    );
  } else {
