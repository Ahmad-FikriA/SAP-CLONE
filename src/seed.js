...
function loadUsersFromExcel() {
  ...
  return rows
    ...
    .map((row) => {
      ...
      const role = rawRole.startsWith("teknisi")
        ? "teknisi"
        : rawRole.startsWith("kasie")
          ? "kasie"
          : rawRole.startsWith("kadis")
            ? "kadis"
            : rawRole || "teknisi";
      const dinas =
        row["Dinas"] && row["Dinas"] !== "-"
          ? String(row["Dinas"]).trim()
          : null;
      const divisi =
...
const equipment = [
  {
    equipmentId: "EQ-TEST-01",
    equipmentName: "[TEST] Pompa Pusat — Dekat & Dalam",
    funcLocId: "A-A1-02-001-001",
    functionalLocation: "Area Pusat Pabrik (Test)",
    category: "Mekanik",
    plantId: "I-22L001",
    plantName: "PS I Cidanau",
    latitude: -6.0135,
    longitude: 106.0219,
  },
  {
    equipmentId: "EQ-TEST-02",
    equipmentName: "[TEST] Pompa Timur — Dalam Pabrik, Jauh",
    funcLocId: "A-A1-02-001-002",
    functionalLocation: "Area Timur Pabrik (Test)",
    category: "Mekanik",
    plantId: "I-22L001",
    plantName: "PS I Cidanau",
    latitude: -6.0117,
  ...
  {
    equipmentId: "EQ-TEST-03",
    equipmentName: "[TEST] Pompa Remote — Luar Pabrik",
    funcLocId: "A-A1-02-002-001",
    functionalLocation: "Area Remote Jauh (Test)",
    category: "Mekanik",
    plantId: "I-22L001",
    plantName: "PS I Cidanau",
    latitude: -6.06,
...
const plants = [
  ...
  {
    plantId: "I-22L005",
    plantName: "Bendung & Jalur Intake",
    shortName: "Bendung",
    city: "Cilegon",
    centerLat: null,
    centerLon: null,
...
async function main() {
  ...
  console.log(
    `  ✓  users        (+${added} added, ${skipped} already existed)`,
  );

  added = 0;
  skipped = 0;
  for (const p of plants) {
    const [, created] = await Plant.findOrCreate({
  ...
  console.log(
    `  ✓  plants       (+${added} added, ${skipped} already existed)`,
  );
  ...
  console.log(
    `  ✓  func_locs    (+${added} added, ${skipped} already existed)`,
  );
  ...
  console.log(
    `  ✓  equipment    (+${added} added, ${skipped} updated/existed)`,
  );
  ...
  console.log(
    `  ✓  task_lists   (+${added} added, ${skipped} already existed)`,
  );
