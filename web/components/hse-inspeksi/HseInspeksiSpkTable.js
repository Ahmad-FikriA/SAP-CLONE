...
import { Search, X, Eye, RefreshCw, AlertCircle, CheckCircle, Clock, XCircle, Trash2, ClipboardList } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { INSPEKSI_STATUS_META } from '@/lib/inspeksi-service';
...
export function HseInspeksiSpkTable({
  schedules = [],
  loading = false,
  usersMap = {},
  onRefresh,
  ...
  // ── Otorisasi Dinas HSE Ketat ──
  const user = getUser();
  const userDinas = user?.dinas?.toLowerCase() || '';
  const isHseMember = userDinas === 'hse' || userDinas.includes('hse');
  const isKadiv = user?.role === 'kadiv';
  const isAdmin = user?.role === 'admin';
  const showExecuteAction = (isHseMember && !isKadiv) || isAdmin;
  ...
  function clearFilters() {
    setSearch(''); setStatusFilter(''); setExecutorFilter('');
  }
  const hasFilter = search || statusFilter || executorFilter;
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 mb-2">
                <span className="text-[10px] font-bold text-gray-400 uppercase">PIC:</span>
                <span className="text-[11px] text-gray-600 font-medium truncate">{resolveExecutorName(s.assignedTo) || '—'}</span>
              </div>
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex gap-1.5" onClick={(e) => e.stopPropagation()}>
                  {showExecuteAction && ACTIVE_STATUSES.has(s.status) && (
                    <Button 
                      variant="outline" 
                      size="sm" 
                      className="h-7 text-xs gap-1 bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100/70"
                      onClick={(e) => { e.stopPropagation(); onExecute?.(s); }}
                    >
                      <ClipboardList size={11} /> Isi Laporan
                    </Button>
                  )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[1100px]">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['No SPK', 'Judul / Objek', 'Lokasi', 'PIC', 'Tanggal Mulai', 'Status', 'Aksi'].map((h) => (
                  <th
                    key={h}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 max-w-[160px] truncate">
                      {resolveExecutorName(s.assignedTo) || <span className="text-gray-300">—</span>}
                    </td>
                      ...
                      <div className="flex items-center gap-1.5">
                        {showExecuteAction && ACTIVE_STATUSES.has(s.status) && (
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-7 text-xs gap-1.5 bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100/70 opacity-80 group-hover:opacity-100"
                            onClick={(e) => { e.stopPropagation(); onExecute?.(s); }}
                          >
                            <ClipboardList size={11} /> Isi Laporan
                          </Button>
